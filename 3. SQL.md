...
### Tabla mostrará todo contenido de la tabla, podemos ser más específicos dandolo solo los campos que se requieren ver.
     SELECT * FROM 
...
###  MOstrará toda la tabla libros

     SELECT titulo, año_escritura FROM libros





###  Mostrará del dni y el nombre y el teléfono de la tabla personas
...
###  Mostrará el dni, el nombre de la persona y qué código de prestamo tiene


     SELECT dni,nombre,id_prestamo
     FROM prestamos INNER JOIN personas on prestamos.dni1=personas.dni;
     
     
  ###   Mostrará el id de autor, el nombre del autor y el título del libro que haya escrito.
...
###  Mostrará el nombre de la persona y el titulo del libro que tiene en prestamo.
...
###  Mostrarpa el nombre de la persona, el titulo del libro y el nombre del escritor del libro que tiene en prestamo
...
### Mostraría los titulos de los libros que hayan sido escritos después del año 2000.


     SELECT titulo,año_escritura
     FROM libros
     WHERE año_escritura > '2000';  
